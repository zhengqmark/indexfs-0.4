...
void create_remove_items_helper(int dirs,
  ...
  /* Uncomment the following line to support random insertion */
...
/******************************************************************************************/
  ...
  /* Uncomment the following lines to support random insertion */
  // for (idx=0; idx<items_per_dir; idx++) {
  //   i = rand_array[idx];
...
/******************************************************************************************/
        ...
        if (IDX_Mkdir(curr_item, DIRMODE) != 0) {
          FAIL("unable to idxfs_mkdir directory");
        }
#else
        if (mkdir(curr_item, DIRMODE) == -1) {
          FAIL("unable to create directory");
 ...
 */
          ...
          FAIL("not implemented"); // DO NOT USE COLLECTIVE CREATES !!
#else
          if (rank == 0 && verbose >= 3) {
            printf( "V-3: create_remove_items_helper (collective): open...\n" );
            fflush( stdout );
...
/******************************************************************************************/
         ...
         */
          ...
          /* if files are shared, use 'open' to create files -- otherwise use 'create' to do so */
          if (shared_file) {
 ...
 */
            ...
            /* IDX_Create is the same as IDX_Mknod -- the returned 'fd' is not valid for future IO ops. */
            if ((fd = IDX_Create(curr_item, FILEMODE)) == -1) {
              FAIL("unable to idxfs_create file");
            }
#else
            if (rank == 0 && verbose >= 3) {
              printf( "V-3: create_remove_items_helper (non-collective, shared): open...\n" );
...
/******************************************************************************************/
        ...
        if ( sync_file ) {
          ...
          /* as for indexfs 0.3.1, fsync is implemented as a non-op */
          if ( IDX_Fsync(fd) == -1 ) {
            FAIL("unable to idxfs_sync file");
          }
        }
#else
        if ( sync_file ) {
...
/******************************************************************************************/
        ...
        if ( IDX_Close(fd) == -1 ) {
          FAIL("unable to idxfs_close file");
        }
#else
        if (rank == 0 && verbose >= 3) {
          printf( "V-3: create_remove_items_helper: close...\n" );
...
/******************************************************************************************/
       ...
       */
        ...
        /* if files are shared, only proc#0 will be responsible for removing files */
...
#elif defined(_HAS_IDXFS)
          if (IDX_Unlink(curr_item) == -1) {
            FAIL("unable to idxfs_unlink file");
          }
#else
          if (unlink(curr_item) == -1) {
            FAIL("unable to unlink file");
...
#elif defined(_HAS_IDXFS)
        if (IDX_Mkdir(temp_path, DIRMODE) != 0) {
          FAIL("unable to idxfs_mkdir directory");
        }
#else
        if (mkdir(temp_path, DIRMODE) == -1) {
...
#elif defined(_HAS_IDXFS)
    if ((rank < path_count) && IDX_Access(testdirpath) != 0) {
      if (IDX_Mkdir(testdirpath, DIRMODE) != 0) {
        FAIL("unable to idxfs_mkdir test directory path");
      }
    }
#else
    if ((rank < path_count) && access(testdirpath, F_OK) != 0) {
...
#elif defined(_HAS_IDXFS)
            if ((rank < path_count) && IDX_Access(testdir) != 0) {
                if (IDX_Mkdir(testdir, DIRMODE) != 0) {
                    FAIL("unable to idxfs_mkdir test directory");
                }
            }
#else
            if ((rank < path_count) && access(testdir, F_OK) != 0) {
