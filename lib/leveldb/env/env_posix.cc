...
#   ifdef POSIX_IO_DEBUG
      fprintf(stderr, "[POSIX] (%s) %s\n", __func__, filename_.c_str());
#   endif
    return s;
  }
...
#   if (defined(OS_LINUX))
      int p[2];
      if (pipe(p) != 0) {
        result = IOError("cannot create pipe", errno);
      } else {
        while(splice(p[0], 0, w_fd, 0,
              splice(r_fd, 0, p[1], 0, 4096, 0), 0) > 0);
        close(p[0]);
        close(p[1]);
      }
#   else
...
#   endif
  ...
  virtual void SleepForMicroseconds(int micros) {
    ...
    nanosleep(&req, &rem); // usleep(micros) is considered obsolete
  }
...
static Env* posix_env = NULL;
static pthread_once_t once = PTHREAD_ONCE_INIT;
namespace {
static void CreatePosixEnv() {
  assert(posix_env == NULL);
  posix_env = new PosixEnv();
}
}
Env* GetOrNewPosixEnv() {
  pthread_once(&once, &CreatePosixEnv); return posix_env;
}
