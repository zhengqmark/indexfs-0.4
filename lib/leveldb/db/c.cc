...
const char* leveldb_iter_internalvalue(const leveldb_iterator_t* iter, size_t* vlen) {
  Slice s = iter->rep->internalvalue();
  *vlen = s.size();
  return s.data();
}
...
leveldb_options_t* leveldb_options_create_with_hdfs_env(const char* ip,
  ...
  option->rep.env = Env::HDFS_Env();
  return option;
}
#endif
#if defined(PVFS)
leveldb_options_t* leveldb_options_create_with_pvfs_env(int buffer_size) {
  leveldb_options_t* option = new leveldb_options_t;
  option->rep.env = Env::PVFSEnv(buffer_size);
  return option;
}
#endif
#endif
...
void leveldb_options_set_max_sst_file_size(leveldb_options_t* opt, int s) {
  opt->rep.max_sst_file_size = s;
}

void leveldb_options_set_level_zero_factor(leveldb_options_t* opt, double s) {
  opt->rep.level_zero_factor = s;
}

void leveldb_options_set_level_factor(leveldb_options_t* opt, double s) {
  opt->rep.level_factor = s;
}

void leveldb_options_disable_compaction(leveldb_options_t* opt) {
  opt->rep.disable_compaction = true;
}
...
leveldb_env_t* leveldb_create_hdfs_env(const char* serverIP,
                                       int serverPort) {
  leveldb_env_t* result = new leveldb_env_t;
  result->rep = Env::HDFS_Env();
  result->is_default = false;
  return result;
}
#endif
#if defined(PVFS)
leveldb_env_t* leveldb_create_pvfs_env(int buffer_size) {
  leveldb_env_t* result = new leveldb_env_t;
  result->rep = Env::PVFSEnv(buffer_size);
  result->is_default = false;
  return result;
}
#endif
...
static InternalFilterPolicy* policy = NULL;
leveldb_tablebuilder_t* leveldb_tablebuilder_create_with_sanitization(
    const leveldb_options_t* options,
    const char* name,
    leveldb_env_t* env,
    char** errptr) {
  ...
  Status s = env->rep->NewWritableFile(std::string(name),
                                       &result->file);
  if (s.ok()) {
    if (options->rep.filter_policy != NULL) {
      Options opt(options->rep);
      policy = new InternalFilterPolicy(options->rep.filter_policy);
      opt.filter_policy = policy;
      result->rep = new TableBuilder(opt, result->file, false);
    } else {
      result->rep = new TableBuilder(options->rep, result->file, false);
    }
  } else {
